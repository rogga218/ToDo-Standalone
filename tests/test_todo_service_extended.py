...
def test_get_todos_filtering(session: Session):
    ...
    todo_service.create_todo(
        session, TodoCreate(title="T1", description="D1", person_id=p1.id, priority=1)
    )
    todo_service.create_todo(
        session, TodoCreate(title="T2", description="D2", person_id=p1.id, priority=2)
    )
    todo_service.create_todo(
        session, TodoCreate(title="T3", description="D3", person_id=p2.id, priority=1)
    )
    ...
    # Filter by both
    todos_combined = todo_service.get_todos(
        session, filter_person_id=p1.id, filter_priority=1
    )
...
def test_subtask_lifecycle(session: Session):
    ...
    todo = todo_service.create_todo(
        session, TodoCreate(title="Parent", description="Desc", person_id=p.id)
    )

    # Create Subtask
    st = todo_service.create_subtask(
        session, SubtaskCreate(title="Sub1", todo_id=todo.id)
    )
...
def test_delete_todo_cascades_subtasks(session: Session):
    ...
    st = todo_service.create_subtask(
        session, SubtaskCreate(title="Child", todo_id=todo.id)
    )
